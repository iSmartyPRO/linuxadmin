...
export function SettingsPage() {
  ...
  const load = async () => {
    const bundle = can('settings', 'read')
      ? await api<{ app: AppConfig }>('/api/settings').catch(() => null)
      : null
    const conn = can('settings_connection', 'read')
      ? await api<{
          db_host: string
          db_port: number
          db_user: string
  ...
  return (
    <div className="la-page" style={{ maxWidth: 960 }}>
      <PageHeader
        title="Settings"
        subtitle="Project options, modules, connection, and panel access (users / roles / API keys)."
        docsKey="settings"
      />
        ...
        <Tabs
          items={[
            ...
            },
            {
              key: 'modules',
              label: 'Modules',
              children: <ModulesSettingsGrid />,
            },
            {
              key: 'access',
              label: 'Access',
              children: <AccessSettingsPanel />,
            },
          ]}
        />
      </Panel>
